...
-- Como o IdCli tem o mesmo nome na tbCliente e tbClientePF precisa especificar de onde vem
...
-- Exercicio 35
select tbCliente.IdCli as "Código", NomeCli as "Nome", CPF as "CPF", RG as "RG", Nasc as "Data de Nascimento"
from tbCliente 
inner join tbClientePF
on tbCliente.IdCli = tbClientePF.IdCli;
...
select IdFornecedor as 'Codigo', NomeFornecedor as 'Fornecedor', telefone as 'Telefone' from tbfornecedor;
select * from vwtbfornecedor;


-- Exercicio 45
select Fornecedor as 'Nome', Telefone from vwtbfornecedor;
...
-- Exercicio 47
select IdCli as 'Código', NomeCli as 'Cliente', CepCli as 'CEP', logradouro as 'Endereço', NumEnd as 'Número', CompEnd as 'Complemento', Bairro, Cidade, Uf as 'UF' from vwtbclientepj;
-- select 'select' as 'select' where (select 'select' = 'select') and ('select' = 'select');


-- Exercicio 48
create view vwtbclientepf as
select tbCliente.IdCli, NomeCli, CPF, RG, RG_Dig, Nasc, CepCli as 'Cep', logradouro, NumEnd, CompEnd, Bairro, Cidade, Uf from tbCliente
inner join tbClientePF on tbCliente.IdCli = tbClientePF.IdCli
inner join tbEndereco on tbCliente.CepCli = tbEndereco.CEP
inner join tbBairro on tbEndereco.IdBairro = tbBairro.IdBairro
inner join tbCidade on tbEndereco.IdCidade = tbCidade.IdCidade
inner join tbUf on tbEndereco.IdUf = tbUf.IdUf;
select * from vwtbclientepf;
